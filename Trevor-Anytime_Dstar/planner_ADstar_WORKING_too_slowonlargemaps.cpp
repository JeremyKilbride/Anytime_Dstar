    ...
    )
{
   ...
   /* if (input_state->xyloc == std::make_pair(robotposeX, robotposeY))
    ...
    if (input_state->xyloc != GoalState->xyloc)
    {
        ...
        double min_rhs = INF;
        for  (std::pair<int, int>succ_loc : input_state->connections)
        {
    ...
    for (double epsilon : all_epsilons)
    {
                ...
                getKey(StartState, epsilon, robotposeX, robotposeY)) || StartState->rhs_val != StartState->g_val||( StartState->rhs_val==INF && StartState->g_val==INF)))
        {
            ...
            if (RS_expand->g_val > RS_expand->rhs_val)
            {
                ...
                for (std::pair<int,int>pred_loc : RS_expand->connections )
                {
        ...
        while (path_found==false) 
        {
            ...
            for (std::pair<int, int>succ_loc : current_state->connections)
            {
                ...
                if (get_transition_cost(robot_map, collision_thresh, succ->xyloc, x_size, y_size) >= collision_thresh)
                {
                    continue;
                }
                double succ_cost = succ->g_val + get_transition_cost(robot_map,collision_thresh,succ->xyloc, x_size, y_size);
    ...
    
{
    ...
    for (std::pair<int, int>change_loc : robot_map_changes)
    {
        ...
        for (std::pair<int, int>pred_loc : changed_state->connections)
        {
                std::shared_ptr<RobotState> RS_pred = find_state(pred_loc, ALL_STATES);
    ...
    int* action_ptr)
{
    ...
    if (curr_time == 0)
    {
       GoalState = find_state(GoalLoc, ALL_STATES);
